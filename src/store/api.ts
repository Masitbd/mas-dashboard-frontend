...
export const api = createApi({
  ...
  tagTypes: ['Posts', 'Comments', 'Newsletter'],
  endpoints: (builder) => ({
    getPosts: builder.query<{ data: Post[]; total: number }, Record<string, string>>({
      query: (params) => ({
        url: '/api/posts',
        params
      }),
    ...
    subscribeNewsletter: builder.mutation<NewsletterSubscriber, { email: string }>({
      query: (body) => ({
