    ...
    <div className="py-12">
      <Container className="grid gap-10 lg:grid-cols-[2.2fr,1fr]">
        <article className="space-y-8">
          ...
          <div className="prose-content text-base" dangerouslySetInnerHTML={{ __html: post.content }} />
          <div className="rounded-2xl bg-accent p-4">
            <div className="flex items-center gap-4 text-xs text-muted">
              <span className="rounded bg-brand px-2 py-1 text-white">Travel</span>
        ...
        <aside className="space-y-8">
            ...
            <div className="mt-4 space-y-4">
              {posts.slice(0, 3).map((author) => (
                <div key={author.author.id} className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{author.author.name}</p>
                    <p className="text-xs text-muted">Fashion Designer, Blogger, Activist</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl bg-brand p-5 text-white">
            <p className="text-sm font-semibold">Want To Travel Sikkim By Car?</p>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="mt-4 space-y-3 text-sm">
              {['Lifestyle', 'Travel', 'Food', 'Healthcare', 'Technology'].map((label, index) => (
                <div key={label} className="flex items-center justify-between">
                  <span>{label}</span>
                  <span className="text-muted">{String(index + 3).padStart(2, '0')}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {['Travel', 'Lifestyle', 'Fashion', 'Technology', 'Business'].map((tag) => (
                <span
                  key={tag}
