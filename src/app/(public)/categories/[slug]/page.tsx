...
export default function CategoryPage({ params }: PageProps) {
  ...
  const category = cd?.data?.data?.[0];
  const categoryId = category?._id as string | undefined;
  ...
  const {
    ...
    { category: categoryId, page: String(page), limit: String(LIMIT) },
    { skip: !categoryId },
  );
  ...
  return (
    <div className="py-12">
      <Container>
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <h1 className="text-4xl font-semibold font-serif text-foreground">
              {catLoading ? (
                <span className="inline-block h-10 w-72 animate-pulse rounded bg-accent align-middle" />
              ) : catError ? (
                "Category"
              ) : category?.name ? (
                <>
            ...
            <p className="mt-2 text-sm text-muted">
              {catFetching || postsFetching ? (
                ...
                "Browse the posts published under this category."
              )}
            </p>
          </div>
        </div>
        ...
        <div className="mt-8">
          {catLoading ? null : !categoryId ? (
            <EmptyState
              title="Category not found"
              subtitle="Please check the URL or try a different category."
            />
          ) : postsError ? (
            <div className="rounded-2xl border border-border bg-card p-10 text-center">
